...
coordCometa1 = [5,20]
coordCometa2 = [random.randint(5,40),60]
coordCometa3 = [random.randint(5,40),105]
animatedObject = True
vidaCometa = [0,0,0]
o1,o2,o3,o4 = 0,0,0,0
cor1,cor2 = 0,0
...
altura1 = random.randint(15,30)
altura2 = random.randint(15,30)
...
def gerarCenarioBase(stdscr,cor1,cor2):
    ...
    for i in range(altura1 ,altura1 + 7):
        for g in range (6,15):
            stdscr.addstr(i,g,'█',curses.color_pair(cor1))
    for k in range(altura1 + 1,altura1 + 6):
            stdscr.addstr(k,15,'█',curses.color_pair(cor1))
    for j in range(altura1 + 2, altura1 + 5):
            stdscr.addstr(j,16,'█',curses.color_pair(cor1))
    for i in range(altura2,altura2 + 7):
        for g in range (111,120):
            stdscr.addstr(i,g,'█',curses.color_pair(cor2))
    for k in range(altura2 +1,altura2 +6):
            stdscr.addstr(k,110,'█',curses.color_pair(cor2))
    for j in range(altura2 + 2,altura2 + 5):
            stdscr.addstr(j,109,'█',curses.color_pair(cor2))
    stdscr.refresh()
    numero = 0
...
def gencoord():
    global objeto1,objeto2,objeto3,objeto4,coords1,coords2,coords3,coords4,cor1,cor2
    ...
    cor1 = random.randint(4,8)
    cor2 = random.randint(4,8)


def gerarCenario(stdscr):
    global objeto1,objeto2,objeto3,objeto4,coords1,coords2,coords3,coords4,o1,o2,o3,o4,coordCometa1,coordCometa2,coordCometa3,vidaCometa,cor1,cor2
    gerarCenarioBase(stdscr,cor1,cor2)
    if animatedObject == True:
        if vidaCometa[0] == 0:
            if coordCometa1[0] < 45:
                stdscr.addstr(coordCometa1[0],coordCometa1[1],'Φ')
                coordCometa1[0] += 1
            if coordCometa1[0] == 45:
                coordCometa1[0] = 5
        if vidaCometa[1] == 0:
            if coordCometa2[0] < 45:
                stdscr.addstr(coordCometa2[0],coordCometa2[1],'Φ')
                coordCometa2[0] += 1
            if coordCometa2[0] == 45:
                coordCometa2[0] = 5
        if vidaCometa[2] == 0:
            if coordCometa3[0] < 45:
